...
class TenderWishlistRepository:
    ...
    def get_wishlist_by_tender_ref(self, tender_ref_number: str, user_id: UUID = None) -> Optional[TenderWishlist]:
        """
        Fetch wishlist entry by tender reference number, optionally filtered by user.
        
        Args:
            tender_ref_number: Reference number from tenders table
            user_id: Optional user ID to filter by (if provided, only returns entry for that user)
            
        Returns:
            TenderWishlist object or None if not found
        """
        query = self.db.query(TenderWishlist).filter(
